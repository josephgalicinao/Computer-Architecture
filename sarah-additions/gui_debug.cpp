...
// Debug window (check if we can get inst info here)
static void debug_window_processor(void)
{
    ...
    // PC PRINTED HERE, SO GONNA WRITE IT TO FILE ALSO
    ImGui::Separator();
    ...
    // save PC trace to a file first
    std::ofstream of;
    of.open("pcTrace.csv", std::ios::app);
    if(!of){ // if file no open
        std::cout << "pcTrace open failed";
    }
    else{ // write PC into file as init test
        of << "0x" << std::hex << proc_state->PC->GetValue() << std::endl;
        of.close();
    }
